...
if __name__ == '__main__':
  ...
  load_name = '/content/drive/MyDrive/Colab Notebooks/img_caption/Faster-R-CNN-with-model-pretrained-on-Visual-Genome/faster_rcnn_res101_vg.pth'
  ...
  SAVE_FT_PATH = '/content/drive/MyDrive/Colab Notebooks/img_caption/ft.h5'
  import pickle
  f = open(SAVE_FT_PATH, 'wb')
  d = {}
  im_files = os.listdir(args.image_dir)[:200]
  import pickle
  with open('/content/drive/MyDrive/Colab Notebooks/img_caption/index.pt', 'wb') as ind:
    pickle.dump(im_files, ind)
  for image_file in tqdm(im_files):
      im_file = os.path.join(args.image_dir, image_file)
    ...
    #   print('pred_boxes: {}, pooled_feat: {}'.format(boxes.shape, keep_fts.shape))
      ...
      d['%s_features' % image_id] = keep_fts.detach().numpy()
  pickle.dump(d, f)
  f.close()