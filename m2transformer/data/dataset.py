...
class Flickr(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root):
        with open(os.path.join(ann_root, 'trains.json'), 'r') as json_file:
            train_json = json.load(json_file)
        with open(os.path.join(ann_root, 'vals.json'), 'r') as json_file:
            val_json = json.load(json_file)
        with open(os.path.join(ann_root, 'tests.json'), 'r') as json_file:
            test_json = json.load(json_file)

        with nostdout():
            self.train_examples, self.val_examples, self.test_examples = self.get_samples(img_root, train_json, val_json, test_json)
        examples = self.train_examples + self.val_examples + self.test_examples
    ...
    @classmethod
    def get_samples(self, img_root, train_json, val_json, test_json):
        train_samples = []
        val_samples = []
        test_samples = []
        
        data_json = {}
        data_json['train'] = train_json
        data_json['val'] = val_json
        data_json['test'] = test_json
    
        for split in ['train', 'val', 'test']:
            for ann in data_json[split]['annotations']:
                caption = ann['caption']
                img_id = ann['image_id']
                filename = ann['image_id'] + '.jpg'
