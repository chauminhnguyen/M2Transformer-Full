...
class ImageDetectionsField(RawField):
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        if delta > 0:
            # print('precomp_data shape: {}, delta: {}'.format(precomp_data.squeeze().shape, delta))
            precomp_data = np.concatenate([precomp_data.squeeze(), np.zeros((delta, precomp_data.shape[-1]))], axis=0)
        elif delta < 0:
