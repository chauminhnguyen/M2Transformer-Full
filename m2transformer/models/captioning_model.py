...
from m2transformer.utils import *
...
class CaptioningModel(Module):
    ...
    def test(self, visual: TensorOrSequence, max_len: int, eos_idx: int, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        b_s = get_batch_size(visual)
        device = get_device(visual)
        outputs = []
        log_probs = []
    ...
    def sample_rl(self, visual: TensorOrSequence, max_len: int, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
        b_s = get_batch_size(visual)
        outputs = []
        log_probs = []
    ...
    def beam_search(self, visual: TensorOrSequence, max_len: int, eos_idx: int, beam_size: int, out_size=1,
